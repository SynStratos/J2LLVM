...
//creazione delle funzioni memcpy e memset
...
Value* ClassNode::codegen()
{
	...
	{
		if(gvar[variables[it]->name]) LogError("already defined variable.", line);
		if(variables[it]->type!="string"){
			...
			LogError("impossible variable definition.", line);
		}
...
Value* IdentifierNode::codegen()
{
	...
	if(!v)
	{
		...
		if(!end) LogError("unknown variable name.", line);
	...
	if(!v) LogError("unknown variable name.", line);
...
{	
	...
	if(innerVariables[name]) LogError("already defined variable.", line);
	if(type=="string"){
		if(value) {
		...
		if(!value) {
			...
			else LogError("wrong type defined.", line);
		}	
	}
}
////////////////////////////////////////////////////////////////////////////////////////
Value* FunctionDec::codegen()
{	
	//controllo che la funzione non sia stata già dichiarata
	if(MyModule->getFunction(name)) LogError("function already defined.", line);
	...
	if(type!="void") return_ptr = Builder.CreateAlloca(typeOf(type), nullptr, "ret0"); 
	...
	if(name!="main"){
		...
		AllocaInst* fp =  Builder.CreateAlloca(pointer1, nullptr, ""); //instance alloca della struct
		...
		if(type!="int" && type!="void") LogError("wrong function type.", line);
		CreateClassRef();
	}
	//////////////////////////////////////////////////////////////////
...
Value* FunctionCall::codegen()
{
	...
	if(!function) LogError("unknown function referenced.", line);

	if(function->arg_size() != args.size()+1) LogError("incorrect arguments passed.", line);
	...
	for( unsigned i=0, e=args.size(); i!=e; i++)
	{
		...
		if(begin_arg->getType() != temp_arg->getType())
		{
			...
			LogError("wrong type of arguments passed.", line);
		}
...
Value* ReturnNode::codegen()
{
	...
	if(ftype_temp=="void") LogError("unexpected return statement in void function.", line);
	if(v_temp->getType() != typeOf(ftype_temp)) LogError("wrong return type.", line);
	return_flag = true;
...
Value* BinOperator::codegen()
{
	...
	if (!L || !R)	LogError("missing operand.", line);
	...
	if( L->getType() == Type::getInt1Ty(MyContext) && R->getType() == Type::getInt1Ty(MyContext))
	{
		switch(op){
			...
			default		: LogError("unavalaible expression.", line);
		}
	}
	...
	{
		switch(op){
			case SUM_T	: return Builder.CreateFAdd(L, R, "addtmp");
			case MIN_T	: return Builder.CreateFSub(L, R, "subtmp");
			case MUL_T	: return Builder.CreateFMul(L, R, "multmp");
			case DIV_T	: return Builder.CreateFDiv(L, R, "divtmp");
			
			case LT_T	: L = Builder.CreateFCmpOLT(L, R, "cmptmp"); return Builder.CreateZExt(L, Type::getInt1Ty(MyContext), "booltmp");
			case GT_T	: L = Builder.CreateFCmpOGT(L, R, "cmptmp"); return Builder.CreateZExt(L, Type::getInt1Ty(MyContext), "booltmp");
	...
	{
		switch(op){
			case SUM_T	: return Builder.CreateAdd(L, R, "addtmp");
			case MIN_T	: return Builder.CreateSub(L, R, "subtmp");
			case MUL_T	: return Builder.CreateMul(L, R, "multmp");
			case DIV_T	: return Builder.CreateUDiv(L, R, "divtmp");
			
			case LT_T	: L = Builder.CreateICmpULT(L, R, "cmptmp"); return Builder.CreateZExt(L, Type::getInt1Ty(MyContext), "booltmp");
			case GT_T	: L = Builder.CreateICmpUGT(L, R, "cmptmp"); return Builder.CreateZExt(L, Type::getInt1Ty(MyContext), "booltmp");
	...
	LogError("unrecognised type of operand.", line);
}
////////////////////////////////////////////////////////////////////////////////////////
Value* Assignment::codegen()
{	
	Value* val = value->codegen();
	if(!val) LogError("missing right operand.", line);
	...
	if(!var){
		...
		ConstantInt* beg = gvar.begin()->second;
		ConstantInt* end = gvar[name];
		if(!end) LogError("undefined variable.", line);
...
Value* IfNode::codegen()
{
	...
	if(Condition->getType()!=Type::getInt32Ty(MyContext) 
		...
		&& Condition->getType()!=Type::getDoubleTy(MyContext)) LogError("illegal type of condition.", line);
...
Value* PrintNode::codegen()
{
	...
	if(!id_v && !string_v && !all_v) LogError("illegal argument type for print function.", line);
	...
	if(string_v)
	{
		string_v->value = string_v->value.substr(1, string_v->value.length()-2);
	...
	if(id_v)
	{
		...
		if(id_call->getType() == PointerType::get(IntegerType::get(MyContext, 8), 0)){
			...
			str_call = StringGen("%d");
			id_call = Builder.CreateLoad(id_call,"");
		}else if (find(stringhe.begin(), stringhe.end(), id_v->name) != stringhe.end()  || gs){
			...
			LogError("illegal argument type for print function.", line);
		}
...
{
	if (find(scanners.begin(), scanners.end(), name) != scanners.end()){
		...
		LogError("no Scanner previously defined.", line);
	}
}
